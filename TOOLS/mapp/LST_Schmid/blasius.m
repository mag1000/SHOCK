function [A,B]=blasius(nosmod,alp,beta,R,D0,D1,D2,D4)
%
...
u=cos(pi*vec/nosmod).^4-2*cos(pi*vec/nosmod).^3+2*cos(pi*vec/nosmod);
u=cos(pi*vec/nosmod).^4-abs(2*cos(pi*vec/nosmod).^3)+abs(2*cos(pi*vec/nosmod));
%du=-2*cos(pi*vec/nosmod);
du=4*cos(pi*vec/nosmod).^3-6*cos(pi*vec/nosmod).^2+2;
du=abs(4*cos(pi*vec/nosmod).^3)-abs(6*cos(pi*vec/nosmod).^2)+2;
du2=12*cos(pi*vec/nosmod).^2-abs(12*cos(pi*vec/nosmod));

% set up Orr-Sommerfeld matrix

B11=D2-ak2*D0;
A11=-(D4-2*ak2*D2+(ak2^2)*D0)/(zi*R);
A11=A11+alp*(u*ones(1,length(u))).*B11+alp*(du2*ones(1,length(du2)))*D0;
er=-200*zi;
