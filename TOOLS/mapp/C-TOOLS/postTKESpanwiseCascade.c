...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt (argc, argv, "f:g:x:s:w:r:z:m:")) != -1)
	{
		switch (c)
		{
			...
			case 'x':
				optionflag+=1;
				Re_deltaStern_vorgabe=atof(optarg);
				break;
			case 's':
				optionflag+=1;
	...
	printf("Starting postTKESpanwiseCascade....\n");
	printf("Die Reynolds-Nummer (y+-Berechnung, option -r (FLOAT)) ist: %g\n",ReynoldsNumber);
	printf("Die Mach-Nummer (option -m (FLOAT)) ist: %g\n",MachNumber);				
	printf("Dieses Tool dient der Erstellung von spannweitigen TKE unter Verwendung einer zeitlich gemittelten und einer transienten Lösung.\n");
	int errorflag=0;
	if(fileflag!=2)
	{
		printf("ERROR: Keine Dateien angegeben (-f 'file_transient.cgns' -g 'file_timeAverage.cgns')!\n");
		errorflag=1;
	}
	
	if(optionflag!=3)
	{
		printf("ERROR: Keine Reynolszahl Re_thetaStern zur Positionsbestimmung angegeben: -x (FLOAT)!\n");
	...
	printf("Die turbulente kinetische Energie wird in der ebene (xi=const.) an der Position bei Re_deltaStern_vorgabe=%f für die lezten %d Samples mit einer Schrittweite von %d ausgewertet.\n ",
	Re_deltaStern_vorgabe,time_samples,time_stepwidth);
	...
	for(i=3./4.*iMeshPoints;i<iMeshPoints;i++)
	{
		...
		if(fabs(Re_deltaStern[i]-Re_deltaStern_vorgabe)<distance)
		{
			distance=fabs(Re_deltaStern[i]-Re_deltaStern_vorgabe);
			i_vorgabe=i;
		}
	}
	printf("Die ermittelte Position ist: x=%f, Re_deltaStern=%f ||| Vorgabe Re_deltaStern_vorgabe=%f\n",Delta[i_vorgabe*6+0],Re_deltaStern[i_vorgabe],Re_deltaStern_vorgabe);	
	...
	float x_vorgabe;
	x_vorgabe=Re_deltaStern_vorgabe;	
	...
	for(i=1./2.*iMeshPoints;i<iMeshPoints;i++)
	{
		...
		if(fabs(x_search[ijk]-x_vorgabe)<distance)
		{
			distance=fabs(x_search[ijk]-x_vorgabe);
			i_vorgabe=i;
		}
	}
	free(x_search);
	printf("Auswertung bei x %g \n ",x_search[0*jMeshPoints*iMeshPoints+0*iMeshPoints+i_vorgabe]);
	///////////////////////////////x-SUCHE-SUCHE-ENDE//////////////////////////	
	}
	...
	irmin3D[0]=i_vorgabe+1;
	irmin3D[1]=1;	
	irmin3D[2]=1;	
	irmax3D[0]=i_vorgabe+1;
	irmax3D[1]=zonesize3D[0][1];
	irmax3D[2]=zonesize3D[0][2];
	...
	printf("Auswertefenster: x:%g, start:%d - ende:%d - samples:%d.\n",x[0],time_start,time_end,time_stepwidth);
	...
	for(solution=time_start;solution<=time_end;solution=solution+time_stepwidth)
	{
		...
		viscLength=calcViscLength(iMeshPoints, jMeshPoints, kMeshPoints, 0, 0, (kMeshPoints-1), u_timeAverage, v_timeAverage, w_timeAverage, p_timeAverage, rho_timeAverage, x, y,ReynoldsNumber);
		
		sprintf(output_TKE,"TKESpanwiseCascade_%d.dat",n);
		file1=fopen(output_TKE,"w");
		fprintf(file1,"TITLE = \"TKE-Spanwise-Cascade\"\n");
		fprintf(file1,"variables = \"z/c [-]\"");
