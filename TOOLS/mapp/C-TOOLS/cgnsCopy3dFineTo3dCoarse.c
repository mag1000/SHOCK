...
void copyFine2Coarse(cgsize_t irmax_coarse[3],int *translationmatrix,float *buffer_coarse,float *buffer_fine)
{
	int i,j,k,ijk;
	for(i=0;i<irmax_coarse[0];i++)
	{
		...
		{
			for(k=0;k<irmax_coarse[2];k++)
			{
				ijk=k*irmax_coarse[1]*irmax_coarse[0]+j*irmax_coarse[0]+i;
...
int main(int argc, char *argv[])
{
	...
	sprintf(file_fine,"/home/mag/DISSERTATION/3D-GKS/8192x512x256/3d_NACA0012_yMin40_500000_8192x512x256_lastSnapshot.cgns");
...
//	Öffnen beider Dateien
	...
	int i_fine,j_fine,k,ijk_fine;
	int i_coarse,j_coarse,ijk_coarse;
	int ijk_correspondig;
	int *translationmatrix;
	translationmatrix=(int*)calloc(irmax_coarse[0]*irmax_coarse[1]*irmax_coarse[2],sizeof(int));
	float distance;
	...
	int k_faktor=zonesize_fine[0][2]/zonesize_coarse[0][2];
	...
	for(i_coarse=0;i_coarse<irmax_coarse[0];i_coarse++)
	{
		for(j_coarse=0;j_coarse<irmax_coarse[1];j_coarse++)
		{
			...
			i_start_fine=i_coarse*i_faktor-500;
			i_end_fine=i_coarse*i_faktor+500;
			j_start_fine=j_coarse*j_faktor-600;
			j_end_fine=j_coarse*j_faktor+600;
